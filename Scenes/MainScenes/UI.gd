...
onready var hp_bar = get_node("HUD/InfoBar/H/HomeHP")
onready var hp_bar_tween = get_node("HUD/InfoBar/H/HomeHP/Tween")
...
func _on_PausePlay_pressed():
	...
	if get_tree().is_paused():
		...
		# not technically paused, just that nothing is happening
...
func update_health_bar(base_health):
	# hp_bar_tween.interpolate_property(node, parameter, start_value, end_value, duration, transistion_type, easing_type)
	hp_bar_tween.interpolate_property(hp_bar, 'value', hp_bar.value, base_health, 0.1, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	# tween interpolates and also sets the new value for the parameter using the end_value
	hp_bar_tween.start()
	if base_health >= 60:
